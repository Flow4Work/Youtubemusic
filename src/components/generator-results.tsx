...
function StaticCardTitle({ title }: { title: string }) {
  return <div className="shrink-0"><h3 className="text-[26px] font-bold tracking-[-0.03em] text-slate-950">{title}</h3></div>;
}
...
export function GeneratorResults(props: Props) {
  ...
  return <section className={`studio-results ${layoutStyles.resultsGrid}`} aria-label="생성 결과">
    ...
    <article className={`result-card style-card ${layoutStyles.styleCompact}`}>
      ...
      </> : <><StaticCardTitle title="Suno에 넣을 스타일"/><EmptyCard text="영어 스타일과 한국어 뜻이 표시됩니다." loading={isInitialLoading}/></>}
    </article>
    ...
    <div className={`publish-stack ${layoutStyles.publishStack}`}>
      <article className={`result-card compact-result-card title-card ${layoutStyles.titleCard}`}>
        ...
        </> : <><StaticCardTitle title="제목"/><EmptyCard text="제목 후보 3개가 표시됩니다." loading={isInitialLoading}/></>}
      </article>
